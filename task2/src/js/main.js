...
require(['backbone', 'app', 'views/filters', 'collections/crimes', 'views/crimes', 'views/maps', 'routers/router'], function(Backbone, app, FiltersView, Crimes, CrimesView, myMapView, Router) {
    crimes = new Crimes();
    crimes.fetch({
        success: function(crimes, response, options) {
            crimesView = new CrimesView({
                collection: crimes
            });
            mapView = new myMapView({
                collection: crimes,
                plots: []
            });
            filterView = new FiltersView({
                options_cats: crimesView.categories
            });
