	...
	var CrimesView = Marionette.CompositeView.extend({
    	...
    	initialize : function (options) {
       		...
       		$(this.collection.models).each( function(index){
	            ...
	            if($.inArray(this.attributes.category, self.categories)===-1)
	            	self.categories.push(this.attributes.category);
       		...
       		app.vent.on('filter:new', function (term){
                ...
                self.filter = function (child, index, collection, term) {
				    ...
				    if(self.term!=="" && self.term !== undefined && self.term !=="Select a category")
				    	return child.get('category') == self.term;
				    else
				    	return 1;	
				}
				self.render();
